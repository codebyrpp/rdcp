...
const defaultSchema: ExclusiveMaxSchema = {
    type: "number",
    exclusiveMaximum: 100,
    errorMessage: {
        exclusiveMaximum: "Exclusive maximum value is 100",
    }
}

type CustomValidationInstance = NumberFieldValidationInstance & {
    schema: ExclusiveMaxSchema;
}
...
function PropertiesComponent({ validationInstance, update }: {
    ...
    function applyChanges(values: z.infer<typeof propertiesSchema>) {
        update({
            ...
            schema: {
                type: schema.type,
                exclusiveMaximum: values.value,
                errorMessage: {
                    exclusiveMaximum: `Exclusive maximum value is ${values.value}`,
                }
            },
        });
    }
...
export const ExclusiveMaxValidation: NumberFieldValidation = {
    type: "exclusiveMax",
    name: "Less than",
